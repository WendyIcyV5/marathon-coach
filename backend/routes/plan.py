...
def create_plan(user_id: int, db: Session = Depends(get_db)):
    ...
    past_races = db.query(models.PastRace).filter(
        models.PastRace.user_id == user_id
    ).all()
    ...
    clean_plan = plan_text.strip().removeprefix("```json").removeprefix("```").removesuffix("```").strip()

    latest_plan = db.query(models.TrainingPlan).filter(
        models.TrainingPlan.user_id == user_id
    ).order_by(models.TrainingPlan.version.desc()).first()
